...
export function useDriverSectorHighlights({
    ...
    const [sectorVisuals, setSectorVisuals] = useState<DriverSectorVisualMap>(() => buildEmptyVisuals(settings));
    ...
    useEffect(() => {
        if (!enabled) {
            ...
            setSectorVisuals(buildStaticVisuals(currentSectors, bestSectors, sessionBestSectors, settings));
    ...
    function activateSector(key: SectorKey, value: number): void {
        const highlightColor = getSectorColor(
            key,
            value,
            bestSectors,
            sessionBestSectors,
