...
import { MOCK_DRIVER, MOCK_SESSION_BEST_SECTORS } from './driverMockData'
...
function findSpectatedDriver(standings: DriverStanding[]): DriverStanding | null {
  return (
    standings.find((standing) => standing.isFocused) ??
...
export function useDriverCardData(): DriverCardData {
  ...
  useEffect(() => {
    ...
    if (isNewDriver) {
      lapTrackerRef.current = {
        ...
        lapStartTime:
          driver.lastLapTime !== null ? Math.max(0, sessionTime - driver.lastLapTime) : sessionTime
      }
      return
    }
  ...
  return {
    ...
    classBestLapTime: isPreview
      ? MOCK_DRIVER.bestLapTime
      : getClassBestLapTime(appState.standings, driver),
