...
export function buildStaticVisuals(
  ...
  return {
    sector1: buildStaticSector(
      'sector1',
      currentSectors.sector1,
      bestSectors,
      sessionBestSectors,
      settings
    ),
    sector2: buildStaticSector(
      'sector2',
      currentSectors.sector2,
      bestSectors,
      sessionBestSectors,
      settings
    ),
    sector3: buildStaticSector(
      'sector3',
      currentSectors.sector3,
      bestSectors,
      sessionBestSectors,
      settings
    )
...
export function clearResetTimer(timerRef: { current: ReturnType<typeof setTimeout> | null }): void {
  if (timerRef.current) {
