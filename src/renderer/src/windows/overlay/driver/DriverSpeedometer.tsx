    ...
    <div style={panelStyle}>
      ...
      <div style={gaugeWrapStyle}>
        ...
        <div style={readoutStyle}>
          <motion.div style={speedValueStyle}>{speedDisplay}</motion.div>
          <div style={speedUnitStyle}>{speedUnitLabel}</div>
          <motion.div style={rpmStyle}>{rpmDisplay.toLocaleString()} RPM</motion.div>
        </div>
      </div>
...
function TopMetric({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      <motion.span style={{ ...valueStyle, color: tone }}>{value}</motion.span>
    </div>
  )
}
