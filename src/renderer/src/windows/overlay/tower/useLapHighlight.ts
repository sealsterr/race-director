...
function resolveLapHighlightColor(
  ...
  const isClassBestLap = classBestLapTime === null ? false : lastLapTime <= classBestLapTime
  if (isClassBestLap) {
    return '#9333ea'
  }

  const isPersonalBest = bestLapTime === null ? false : lastLapTime <= bestLapTime
  if (isPersonalBest) {
    return '#16a34a'
  }
...
export function useLapHighlight({
  ...
  useEffect(() => {
    ...
    if (enabled && lastLapTime !== null && lastLapTime > 0 && lastLapTime !== previousLastLap) {
      setLapHighlightTime(lastLapTime)
      setLapHighlightColor(resolveLapHighlightColor(lastLapTime, bestLapTime, classBestLapTime))
