...
import { TYRE_INFO, getTyreColor, normalizeTyreCompound, type TyreCompoundKey } from './constants'
...
function getTyreLayout(tyreCompound: TyreCompound, tyreSet: TyreSet | null): TyreCompoundKey[] {
  if (tyreSet) {
    return [
...
export default function TyreDisplay({ tyreCompound, tyreSet, settings }: TyreDisplayProps) {
    ...
    <div style={footprintStyle}>
      ...
      >
        {layout.map((compound, index) => (
          <TyreGlyph key={`${compound}-${index}`} compound={compound} settings={settings} compact />
        ))}
      </div>
    </div>
  )
