...
interface FightGroupBlockProps {
  ...
  readonly rowPositionByKey: Map<
    string,
    { readonly isFirstRow: boolean; readonly isLastRow: boolean }
  >
}
      ...
      <div style={{ position: 'relative', borderRadius: 4, overflow: 'visible' }}>
        ...
        {rows.map((row) => (
          <TowerRow
            ...
            row={row}
            settings={settings}
            isFirstRow={rowPositionByKey.get(row.key)?.isFirstRow ?? false}
            isLastRow={rowPositionByKey.get(row.key)?.isLastRow ?? false}
