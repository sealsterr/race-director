...
export function getClassColor(carClass: CarClass, settings: TowerSettings): string {
  switch (carClass) {
    case 'HYPERCAR':
...
export interface TyreInfo {
  ...
  defaultColor: string
  settingsKey:
    | keyof Pick<TowerSettings, 'colorHard' | 'colorMedium' | 'colorSoft' | 'colorWet'>
    | null
}
...
export function getTyreColor(compound: TyreCompoundKey, settings: TowerSettings): string {
...
export function getFightLabel(classPosition: number): FightLabel {
