...
function getActiveStandings(standings: DriverStanding[]): DriverStanding[] {
  return standings.filter(
    (standing) => standing.status !== 'RETIRED' && standing.status !== 'DISQUALIFIED'
  )
}

function getVisibleClassRows(standings: DriverStanding[], settings: TowerSettings): number[] {
  const rowsByClass = new Map<string, number>()

  getActiveStandings(standings).forEach((standing) => {
    rowsByClass.set(standing.carClass, (rowsByClass.get(standing.carClass) ?? 0) + 1)
  })
...
export function getTowerBaseHeight(standings: DriverStanding[], settings: TowerSettings): number {
