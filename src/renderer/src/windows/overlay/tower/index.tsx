/* eslint-disable @typescript-eslint/explicit-function-return-type, react-hooks/refs */
...
function getStatusEarGutter(sections: ReturnType<typeof useTowerData>['sections']): number {
  const hasVisibleStatusEar = sections.some((section) =>
    section.rows.some(
      (row) => row.standing.status === 'PITTING' || row.standing.status === 'FINISHED'
    )
  )
...
function applyOvertakeFlash(
  gained: number[],
  lost: number[],
  setOvertakingSlots: React.Dispatch<React.SetStateAction<Map<number, 'gained' | 'lost'>>>,
  overtakeTimersRef: React.RefObject<Map<number, ReturnType<typeof setTimeout>>>
): void {
...
export default function TowerOverlay() {
  ...
  const fightEnabled =
    ...
    (!towerSettings.fightOnlyInIntervalMode || towerSettings.raceMode === 'GAP_AHEAD') &&
  ...
  const sessionBestSectors = isQuali ? deriveSessionBestSectors(effectiveStandings) : EMPTY_SECTORS
        ...
        {sections.map((section, index) => (
          <TowerSection
            ...
            fightGroups={fightGroups.filter((g) =>
              section.rows.some((r) => g.slotIds.includes(r.standing.slotId))
            )}
