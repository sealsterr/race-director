...
const updateWindowOpenStatus = (windows: WindowItem[], id: string, isOpen: boolean): WindowItem[] =>
  windows.map((windowItem) => (windowItem.id === id ? { ...windowItem, isOpen } : windowItem))
...
const useDashboardIpcSync = ({
  ...
  useEffect(() => {
    ...
    const unsubConn = globalThis.api.onConnectionChange((status) => {
      ...
      const messages: Record<typeof status, [string, LogType]> = {
        ...
        ERROR: ['Connection failed! Make sure Le Mans Ultimate is running.', 'ERROR']
      }
    ...
    const unsubWinClosed = globalThis.api.windows.onClosed((id: string) => {
      ...
      const overlayIds = ['OVERLAY-TOWER', 'OVERLAY-DRIVER', 'OVERLAY-GAP', 'OVERLAY-SESSION']
  ...
  }, [addLog, closeOverlaysWhenControlCloses, setConnection, setSession, setStandings, setWindows])
}
