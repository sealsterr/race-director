  ...
  return (
    ...
    >
      <div className="min-w-0 flex-1">
        <p id={ids.labelId} className="flex items-center gap-2 text-sm font-medium text-rd-text">
          <span>{label}</span>
          {badgeLabel ? <SettingsBadge label={badgeLabel} /> : null}
        </p>
        <p id={ids.descriptionId} className="mt-1 text-xs leading-5 text-rd-muted">
          {description}
        </p>
      </div>
...
const SettingsSectionTitle = ({ title }: SettingsSectionTitleProps): React.ReactElement => {
  return (
    <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-rd-subtle">{title}</p>
  )
}

export { SectionBlock, SettingsRow, SettingsSectionTitle, SettingsToggle }
