...
const GeneralThemeSection = ({
  ...
  const isCustomSelected =
    settings.general.accentPreset === CUSTOM_ACCENT_PRESET_ID;
  const activeField =
    CUSTOM_COLOR_FIELDS.find((field) => field.key === activeKey) ??
  ...
  useLayoutEffect(() => {
    ...
    const updatePosition = (): void => {
      ...
      const measuredHeight = Math.min(
        maxHeight,
        popoverRef.current?.offsetHeight ?? 340
      );
      const left = Math.min(
      ...
      setPickerStyle({
        ...
        top: openAbove
          ? Math.max(
              VIEWPORT_MARGIN_PX,
              rect.top - measuredHeight - POPOVER_MARGIN_PX
            )
          : Math.max(
              VIEWPORT_MARGIN_PX,
              Math.min(
  ...
  return (
    <SectionBlock title="Theme">
      <SettingsRow
        label="Palette"
        description="Select a built-in palette or use the custom swatch to tune your own color set."
      >
            ...
            {ACCENT_PRESETS.map((preset) => {
              ...
              return (
                <button
                  ...
                  title={preset.label}
                  onClick={() =>
                    onChange((prev) => updateAccentPreset(prev, preset.id))
                  }
              ...
              <div className="max-h-full overflow-y-auto rounded-2xl">
                <div className="rounded-2xl border border-rd-border bg-rd-surface/96 p-1.5 shadow-[0_24px_60px_rgba(0,0,0,0.5)] backdrop-blur">
                  ...
                  <ColorPickerPanel
                    ...
                    onChange={(value) =>
                      onChange((prev) =>
                        updateCustomPaletteColor(prev, activeField.key, value)
                      )
                    }
                  />
                </div>
