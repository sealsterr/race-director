...
function sanitizeDraft(raw: string, allowDecimal: boolean, allowNegative: boolean): string {
...
const CustomNumberField = ({
  ...
  const inputStyle = useMemo<CSSProperties | undefined>(() => {
    ...
    const liveValue = isFocused ? draft : String(value)
    const width = Math.min(10, Math.max(3, liveValue.length === 0 ? 3 : liveValue.length + 1))
    return { width: `${width}ch` }
  }, [draft, isFocused, value, widthMode])
