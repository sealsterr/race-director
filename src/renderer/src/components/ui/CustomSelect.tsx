...
const DEFAULT_OPTION_CLASS =
  "px-3 py-2 text-sm text-rd-text transition-colors";
...
const CustomSelect = ({
  ...
  useLayoutEffect(() => {
    ...
    const updatePosition = (): void => {
      ...
      const availableBelow =
        window.innerHeight - rect.bottom - VIEWPORT_MARGIN_PX - MENU_MARGIN_PX;
      ...
      setMenuStyle({
        left: rect.left,
        top: openAbove
          ? rect.top - maxHeight - MENU_MARGIN_PX
  ...
  useEffect(() => {
    if (selectedIndex < 0 || isOpen) return;
